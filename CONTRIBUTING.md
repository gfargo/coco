...
The scenario library (named, deterministic temp git repo states) lives in `packages/git-scenarios/` — a private in-monorepo package being shadow-extracted toward a standalone `@gfargo/git-scenarios` npm release. Import via the package name: `import { spinUpScenario } from '@gfargo/git-scenarios'`.
...
See `packages/git-scenarios/README.md` for the full list of scenarios, the programmatic API, and the discipline for adding new scenarios.
