...
function checkServiceBlock(config: Config, diagnostics: Diagnostic[]) {
  ...
  if (!config.service.provider) {
    diagnostics.push({
      ...
      fix: 'Set service.provider to "openai", "anthropic", "gemini", "mistral", or "ollama".',
    })
  }
...
function checkAuthentication(config: Config, diagnostics: Diagnostic[]) {
  ...
  if (!authentication || authentication.type === 'None') {
    diagnostics.push({
      ...
      fix: `Set service.authentication to { "type": "APIKey", "credentials": { "apiKey": "..." } } or use the OPENAI_API_KEY / ANTHROPIC_API_KEY / GEMINI_API_KEY / MISTRAL_API_KEY environment variable.`,
    })
    return
  }
  ...
  if (authentication.type === 'APIKey') {
    ...
    if (!key || key === '•••••••••••••••' || key.trim() === '') {
      diagnostics.push({
        ...
        fix: `Set the API key in your config or via environment variable (OPENAI_API_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY, or MISTRAL_API_KEY).`,
      })
    }
  }
}
