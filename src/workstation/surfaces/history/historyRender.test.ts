...
describe('renderHistoryPanel', () => {
  ...
  describe('active-row caret (redundant non-color cue)', () => {
    // Flatten every string descendant of the rendered tree into one blob.
    ...
    it('renders the ❯ caret on the selected commit row', () => {
      const out = flattenText(render(makeState()))
      expect(out).toContain('❯ ')
    })

    it('falls back to the ascii "> " caret and drops the unicode glyph', () => {
      const out = flattenText(render(makeState(), { ascii: true }))
      expect(out).not.toContain('❯')
    })

    it('still shows the caret with a no-color theme (the NO_COLOR cue)', () => {
      // Render through the real renderer but force a no-color theme so the
      // selection background is suppressed — the caret is then the only
      // visual cue that survives, which is the whole point of this feature.
      const tree = renderHistoryPanel(
        createElement,
        components,
        makeState(),
        baseContext,
        30,
        120,
        createLogInkTheme({ noColor: true }),
        false,
        false,
        'normal',
        'single',
