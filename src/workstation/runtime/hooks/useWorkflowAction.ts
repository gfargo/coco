 ...
 * reproduced byte-for-byte from `app.ts`. The inline helper `runApplyHunk`
 * stays INLINE inside the moved callback (the issue / PR cache-invalidation
 * helpers moved out to `useForgeTriageWorkflowActions`, see below), the
 * `handlers` object is unchanged, and the post-handler logic carries over
 * verbatim. To keep the body + dep-array text identical, the big
...
import { LogInkContextStatus } from '../../chrome/context'
import { openProviderUrl } from '../../../git/providerActions'
import type { GitProviderType } from '../../../git/providerData'
import { getSelectedBranchId, getSelectedBranch, getSelectedBranchBatch, getSelectedTagId, getSelectedTag, getSelectedStash, getSelectedStashBatch, getSelectedCommitRange, isContiguousHistoryRange, getSelectedWorktreeId, getSelectedWorktree, getSelectedPullRequestTriageId } from '../selection'
import {
...
export function useWorkflowAction(
  ...
  const runWorkflowAction = React.useCallback(async (id: string, payload?: string) => {
    ...
    const handlers: Record<string, () => Promise<{ ok: boolean; message: string } | undefined>> = {
      ...
      // #783 / #882 — forge PR-action-panel + issue/PR-triage handlers,
      // extracted to ./useForgeTriageWorkflowActions (OSS-948).
      ...createForgeTriageWorkflowHandlers({
        forge,
        forgeProvider,
        state,
        context,
